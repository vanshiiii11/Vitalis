...
# Edit DATABASE_URL, JWT secrets, and any of ANTHROPIC_API_KEY / SMTP_* / GOOGLE_* you have
```
...
| Role    | Email                    | Password     |
|---------|---------------------------|--------------|
| Patient | patient@vitalis.app       | Patient@123  |
| Doctor  | dr.smith@vitalis.app      | Doctor@123   |
| Doctor  | dr.chen@vitalis.app       | Doctor@123   |
| Admin   | admin@vitalis.app         | Admin@123    |
...
| `ANTHROPIC_API_KEY` | ❌ | Enables real AI pre/post-visit summaries (Claude). Without it, the app falls back to a deterministic template — booking still works. |
...
| GET | `/api/doctors/:id/slots?date=YYYY-MM-DD` | No | Available slots for date (past slots on today excluded) |
...
| POST | `/api/appointments/:id/confirm` | PATIENT | Confirm held appointment (requires symptom form submitted) |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/api/appointments/:id/symptoms` | PATIENT | Submit symptom form (triggers LLM pre-visit summary) |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/api/appointments/:id/notes` | DOCTOR | Submit post-visit notes + prescription (triggers LLM patient-friendly summary) |
...
| POST | `/api/admin/leave` | Create doctor leave (triggers conflict reconciliation + patient notification) |
...
**Failure handling:** All LLM calls are wrapped in try/catch with a timeout and retry attempts. On failure, `llmStatus` is set to `FAILED` and the UI shows "AI summary unavailable — please review raw symptoms/notes below," falling back to a deterministic template rather than blocking the flow. Booking and visit workflows are **never blocked** by LLM availability.
...
6. Configure the OAuth consent screen: APIs & Services → OAuth consent screen
   - User type: External
   - Add test users (your own email addresses, while the app is unpublished)
   - Add scope: `https://www.googleapis.com/auth/calendar.events`
7. In the app, patients and doctors each independently connect their calendar via their profile page ("Connect Google Calendar") — connecting is optional; booking works without it, calendar sync simply won't fire for that user until connected.
...
3. New → Blueprint → connect your repo → select `render.yaml`
4. Render auto-provisions PostgreSQL and the Node.js service
5. After deploy, run the seed once: Render Dashboard → your API service → Shell → `npm run db:seed`
6. Set the optional env vars (`ANTHROPIC_API_KEY`, `SMTP_*`, `GOOGLE_*`) in the Render Dashboard
...
3. Add environment variable: `VITE_API_URL` = your Render API URL (e.g. `https://vitalis-api.onrender.com/api`)
4. Deploy
...
|---|---|---|
| `hold-expiry-sweeper` | Every 2 min | Release expired HELD slots back to availability |
| `notification-retry` | Every 5 min | Retry FAILED notifications with exponential backoff, up to `MAX_NOTIFICATION_RETRIES` |

Both implement a common job-queue interface — swap to BullMQ by implementing a BullMQ-backed queue driver if Redis becomes available in your deployment environment.
